...
def retrieval_image(query_path, method, dataset="oxford"):
    preloadData(dataset)
    if method == "delf":
    ...
    elif method == "cnn_pytorch":
        if __name__ == "__main__":
            feature_corpus, net, transform, ms = CNNPreload()
            scores = CNNSearch(query_path, feature_corpus, net, transform, ms)
    elif method == "cnn":
        scores = ResNetSearch(query_path)
